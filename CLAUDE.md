...
npm run provider -- google-slides            # regenerate this repo
npm run provider -- google-slides --check    # exit 1 on any difference
```
...
| `packages/ui` | `@particle-academy/google-slides-ui` — the authoring surface, React, every host | Generated |
| `packages/js` | `@particle-academy/google-slides-js` — Node, on `@particle-academy/fancy-connector-core` | Generated |
| `packages/php` | `particle-academy/google-slides-php` — PHP 8.4, on `particle-academy/fancy-connector-core` | Generated |
| `packages/py` | `fancy-google-slides` — Python 3.11+, stdlib only | Generated |

The authoritative copy of `provider/` lives in the weaver envelope at
`providers/google-slides/provider/`. The copy here is emitted from it and `--check`
...
| | Namespace |
|---|---|
| This repo | `github.com/Fancy-Friends/google-slides` |
| npm | `@particle-academy/google-slides-ui`, `@particle-academy/google-slides-js` |
| Packagist | `particle-academy/google-slides-php` |
| PyPI | `fancy-google-slides` |
...
**Default to plain HTTP.** A Google Slides SDK is third-party code and is subject to
